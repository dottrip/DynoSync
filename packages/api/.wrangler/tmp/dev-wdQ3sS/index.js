...
auth.post("/register", async (c) => {
  ...
  const { data, error: error3 } = await supabase.auth.signUp({
    email,
    password,
  ...
  if (data.user) {
    ...
    await adminSupabase.from("users").insert({ id: data.user.id, email, username });
  }
...
vehicles.post("/", async (c) => {
  ...
  const { data, error: error3 } = await supabase.from("vehicles").insert({ user_id: userId, make, model, year, trim, drivetrain }).select().single();
  if (error3) return c.json({ error: error3.message }, 500);
...
vehicles.patch("/:id", async (c) => {
  ...
  const { data, error: error3 } = await supabase.from("vehicles").update({ make, model, year, trim, drivetrain, is_archived, is_public, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", id).eq("user_id", userId).select().single();
  if (error3 || !data) return c.json({ error: "Not found" }, 404);
  return c.json(data);
});
...
// .wrangler/tmp/bundle-sX7lNE/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-sX7lNE/middleware-loader.entry.ts
