...
// ../../node_modules/.pnpm/@cloudflare+unenv-preset@2._8f5be261816f5399d055d8dcd47995b8/node_modules/@cloudflare/unenv-preset/dist/runtime/polyfill/performance.mjs
...
// ../../node_modules/.pnpm/@cloudflare+unenv-preset@2._8f5be261816f5399d055d8dcd47995b8/node_modules/@cloudflare/unenv-preset/dist/runtime/node/console.mjs
...
// ../../node_modules/.pnpm/wrangler@4.68.1_@cloudflare+workers-types@4.20260305.0/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-console
...
// ../../node_modules/.pnpm/@cloudflare+unenv-preset@2._8f5be261816f5399d055d8dcd47995b8/node_modules/@cloudflare/unenv-preset/dist/runtime/node/process.mjs
...
// ../../node_modules/.pnpm/wrangler@4.68.1_@cloudflare+workers-types@4.20260305.0/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-process
...
// ../../node_modules/.pnpm/hono@4.12.3/node_modules/hono/dist/utils/color.js
...
// ../../node_modules/.pnpm/hono@4.12.3/node_modules/hono/dist/middleware/logger/index.js
...
auth.post("/login", async (c) => {
  ...
  const { data, error: error3 } = await supabase.auth.signInWithPassword({ email, password });
  if (error3) return c.json({ error: error3.message }, 401);
...
auth.post("/google", async (c) => {
  ...
  if (error3) return c.json({ error: error3.message }, 400);
...
var vehicles = new Hono2();
vehicles.use("*", authMiddleware);
vehicles.get("/", async (c) => {
  const userId = c.get("userId");
  const supabase = createClient(c.env.SUPABASE_URL, c.env.SUPABASE_SERVICE_ROLE_KEY);
  const { data, error: error3 } = await supabase.from("vehicles").select("*").eq("user_id", userId).order("created_at", { ascending: false });
...
vehicles.post("/", async (c) => {
  ...
  if (limit !== Infinity) {
    const { count: count3 } = await supabase.from("vehicles").select("*", { count: "exact", head: true }).eq("user_id", userId).eq("is_archived", false);
    if ((count3 ?? 0) >= limit) {
  ...
  const { data, error: error3 } = await supabase.from("vehicles").insert({
...
vehicles.get("/:id", async (c) => {
  ...
  const supabase = createClient(c.env.SUPABASE_URL, c.env.SUPABASE_SERVICE_ROLE_KEY);
  const { data, error: error3 } = await supabase.from("vehicles").select("*, dyno_records(*), mod_logs(*)").eq("id", id).eq("user_id", userId).single();
...
vehicles.patch("/:id", async (c) => {
  ...
  const { data, error: error3 } = await supabase.from("vehicles").update({ make, model, year, trim, drivetrain, image_url, is_archived, is_public, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", id).eq("user_id", userId).select().single();
  if (error3 || !data) return c.json({ error: "Not found" }, 404);
  return c.json(data);
});
vehicles.delete("/:id", async (c) => {
  ...
  const id = c.req.param("id");
  const supabase = createClient(c.env.SUPABASE_URL, c.env.SUPABASE_SERVICE_ROLE_KEY);
  const { data, error: error3 } = await supabase.from("vehicles").update({ is_archived: true, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", id).eq("user_id", userId).select().single();
  if (error3 || !data) return c.json({ error: "Not found" }, 404);
  return c.json({ success: true });
});
...
dyno.get("/:vehicleId", async (c) => {
  ...
  const supabase = createClient(c.env.SUPABASE_URL, c.env.SUPABASE_SERVICE_ROLE_KEY);
  const { data: vehicle } = await supabase.from("vehicles").select("id").eq("id", vehicleId).eq("user_id", userId).single();
  if (!vehicle) return c.json({ error: "Vehicle not found" }, 404);
  const { data, error: error3 } = await supabase.from("dyno_records").select("*").eq("vehicle_id", vehicleId).order("recorded_at", { ascending: false });
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json(data);
});
dyno.post("/:vehicleId", async (c) => {
  ...
  const { data: user } = await supabase.from("users").select("tier").eq("id", userId).single();
  const tier = user?.tier ?? "free";
  const limit = TIER_LIMITS[tier].dynoRecords;
  if (limit !== Infinity) {
    const { count: count3 } = await supabase.from("dyno_records").select("*", { count: "exact", head: true }).eq("vehicle_id", vehicleId);
    if ((count3 ?? 0) >= limit) {
  ...
  const { data, error: error3 } = await supabase.from("dyno_records").insert({
    id: crypto.randomUUID(),
...
dyno.get("/:vehicleId/:id", async (c) => {
  ...
  const { data: vehicle } = await supabase.from("vehicles").select("id").eq("id", vehicleId).eq("user_id", userId).single();
  if (!vehicle) return c.json({ error: "Vehicle not found" }, 404);
  const { data, error: error3 } = await supabase.from("dyno_records").select("*").eq("id", id).eq("vehicle_id", vehicleId).single();
  if (error3 || !data) return c.json({ error: "Not found" }, 404);
  return c.json(data);
});
dyno.delete("/:vehicleId/:id", async (c) => {
  ...
  const { data: vehicle } = await supabase.from("vehicles").select("id").eq("id", vehicleId).eq("user_id", userId).single();
  if (!vehicle) return c.json({ error: "Vehicle not found" }, 404);
  const { error: error3 } = await supabase.from("dyno_records").delete().eq("id", id).eq("vehicle_id", vehicleId);
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json({ success: true });
});
...
mods.get("/:vehicleId", async (c) => {
  ...
  const { data: vehicle } = await supabase.from("vehicles").select("id").eq("id", vehicleId).eq("user_id", userId).single();
  if (!vehicle) return c.json({ error: "Vehicle not found" }, 404);
  const { data, error: error3 } = await supabase.from("mod_logs").select("*").eq("vehicle_id", vehicleId).order("installed_at", { ascending: false });
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json(data);
});
mods.post("/:vehicleId", async (c) => {
  ...
  const { data: user } = await supabase.from("users").select("tier").eq("id", userId).single();
  const tier = user?.tier ?? "free";
  const limit = TIER_LIMITS[tier].modLogs;
  if (limit !== Infinity) {
    const { count: count3 } = await supabase.from("mod_logs").select("*", { count: "exact", head: true }).eq("vehicle_id", vehicleId);
    if ((count3 ?? 0) >= limit) {
  ...
  const { data, error: error3 } = await supabase.from("mod_logs").insert({
    id: crypto.randomUUID(),
    vehicle_id: vehicleId,
    category,
    description,
    cost,
    installed_at: installed_at ?? (/* @__PURE__ */ new Date()).toISOString(),
    created_at: (/* @__PURE__ */ new Date()).toISOString()
  }).select().single();
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json(data, 201);
...
mods.get("/:vehicleId/:id", async (c) => {
  ...
  const { data: vehicle } = await supabase.from("vehicles").select("id").eq("id", vehicleId).eq("user_id", userId).single();
  if (!vehicle) return c.json({ error: "Vehicle not found" }, 404);
  const { data, error: error3 } = await supabase.from("mod_logs").select("*").eq("id", id).eq("vehicle_id", vehicleId).single();
  if (error3 || !data) return c.json({ error: "Not found" }, 404);
  return c.json(data);
});
mods.delete("/:vehicleId/:id", async (c) => {
  ...
  const { data: vehicle } = await supabase.from("vehicles").select("id").eq("id", vehicleId).eq("user_id", userId).single();
  if (!vehicle) return c.json({ error: "Vehicle not found" }, 404);
  const { error: error3 } = await supabase.from("mod_logs").delete().eq("id", id).eq("vehicle_id", vehicleId);
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json({ success: true });
});
...
leaderboard.get("/", async (c) => {
  ...
  let query = supabase.from("dyno_records").select(`
      id,
      whp,
      torque_nm,
      zero_to_sixty,
    ...
    `).neq("vehicles.users.tier", "free").eq("vehicles.is_archived", false).eq("vehicles.is_public", true);
  if (region === "following") {
    const { data: followsData } = await supabase.from("build_follows").select("vehicle_id").eq("user_id", userId);
...
publicRoutes.get("/vehicles/:id", async (c) => {
  const id = c.req.param("id");
  const supabase = createClient(c.env.SUPABASE_URL, c.env.SUPABASE_SERVICE_ROLE_KEY);
  const { data, error: error3 } = await supabase.from("vehicles").select("*, dyno_records(*), mod_logs(*), users(username, instagram_handle, discord_id)").eq("id", id).eq("is_public", true).eq("is_archived", false).single();
  if (error3 || !data) return c.json({ error: "Vehicle not found or not public" }, 404);
  return c.json(data);
});

// src/routes/follows.ts
var follows = new Hono2();
follows.use("*", authMiddleware);
follows.get("/check/:vehicleId", async (c) => {
  const userId = c.get("userId");
  const vehicleId = c.req.param("vehicleId");
  const supabase = createClient(c.env.SUPABASE_URL, c.env.SUPABASE_SERVICE_ROLE_KEY);
  const { data, error: error3 } = await supabase.from("build_follows").select("id").eq("user_id", userId).eq("vehicle_id", vehicleId).single();
...
follows.post("/:vehicleId", async (c) => {
  ...
  const { data, error: error3 } = await supabase.from("build_follows").insert([{ id: crypto.randomUUID(), user_id: userId, vehicle_id: vehicleId }]).select().single();
  if (error3) {
...
follows.delete("/:vehicleId", async (c) => {
  ...
  const vehicleId = c.req.param("vehicleId");
  const supabase = createClient(c.env.SUPABASE_URL, c.env.SUPABASE_SERVICE_ROLE_KEY);
  const { error: error3 } = await supabase.from("build_follows").delete().eq("user_id", userId).eq("vehicle_id", vehicleId);
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json({ success: true });
});
...
feedback.post("/", async (c) => {
  ...
  const { error: error3 } = await supabase.from("feedback").insert({
    id: crypto.randomUUID(),
    user_id: userId,
    type,
...
profile3.use("*", authMiddleware);
profile3.get("/me", async (c) => {
  const userId = c.get("userId");
  const supabase = createClient(c.env.SUPABASE_URL, c.env.SUPABASE_SERVICE_ROLE_KEY);
  const { data, error: error3 } = await supabase.from("users").select("id, email, username, tier, avatar_url, instagram_handle, discord_id, created_at").eq("id", userId).single();
  if (error3 || !data) return c.json({ error: "User not found" }, 404);
  return c.json(data);
});
profile3.patch("/me", async (c) => {
  const userId = c.get("userId");
  ...
  if (body.avatar_url !== void 0) updates.avatar_url = body.avatar_url;
  if (body.push_token !== void 0) updates.push_token = body.push_token;
  if (body.instagram_handle !== void 0) updates.instagram_handle = body.instagram_handle;
  if (body.discord_id !== void 0) updates.discord_id = body.discord_id;
  let { data, error: error3 } = await supabase.from("users").update(updates).eq("id", userId).select("id, email, username, tier, avatar_url, instagram_handle, discord_id, created_at");
  if (!error3 && (!data || data.length === 0)) {
    const { data: upsertData, error: upsertError } = await supabase.from("users").upsert({
      ...
      updated_at: (/* @__PURE__ */ new Date()).toISOString(),
      ...updates
    }, { onConflict: "id" }).select("id, email, username, tier, avatar_url, instagram_handle, discord_id, created_at");
    data = upsertData;
    error3 = upsertError;
  }
  if (error3) return c.json({ error: error3.message ?? "Update failed" }, 500);
...
app.route("/profile", profile3);
var src_default = app;

// ../../node_modules/.pnpm/wrangler@4.68.1_@cloudflare+workers-types@4.20260305.0/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../node_modules/.pnpm/wrangler@4.68.1_@cloudflare+workers-types@4.20260305.0/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-2y4F56/middleware-insertion-facade.js
...
// ../../node_modules/.pnpm/wrangler@4.68.1_@cloudflare+workers-types@4.20260305.0/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-2y4F56/middleware-loader.entry.ts
